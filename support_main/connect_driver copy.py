    ...
    def thread_sent_data_driver(self):
        while self.close == 0:
            ...
            if self.sent_data_driver == 0:
                ...
                if time.time() - self.time_sent > 0.1 and self.time_sent != 0:
                    self.time_sent = time.time()
                    ...
                    if self.dk_agv_thu_cong == 0:
                        ...
                        if self.vt_phai_sent == 0 and self.vt_trai_sent == 0:
                            if self.time_stop_dk_tay == 0:
                                self.time_stop_dk_tay = time.time()
                                v_trai_gui = int(v_trai_backup)
                                v_phai_gui = int(v_phai_backup)
                            else:
                                ty_le = (time.time() - self.time_stop_dk_tay)/1
                                if ty_le >= 1:
                                    v_trai_gui = 0
                                    v_phai_gui = 0
                                else:
                                    v_trai_gui = int(v_trai_backup * (1 - ty_le))
                                    v_phai_gui = int(v_phai_backup * (1 - ty_le)) 
                        else:
                            self.time_stop_dk_tay = 0

                            delta_v_trai = abs(self.vt_trai_sent - v_trai_backup)
                            delta_v_phai = abs(self.vt_phai_sent - v_phai_backup)

                            k = self.vt_phai_sent * self.vt_trai_sent
...
if __name__ == "__main__":
    ...
    # # Clear alarm & enable
    # driver_motor.clear_alarm()
    # driver_motor.set_mode(3)     # velocity
    # driver_motor.enable_motor()
    ...
    # # Thử chạy
    # driver_motor.set_rpm(200, 200)   # cho cả 2 bánh chạy 200 rpm
    # time.sleep(5)
    # driver_motor.set_rpm(0, 0)
    # time.sleep(2)
    # driver_motor.disconnect()
