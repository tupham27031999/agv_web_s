...
class Python_Esp:
    ...
    def load_data(self):
        ...
        if self.connected == True and self.close_all == 0:
                ...
                if self.out != "":
                    if len(str(self.out).split("#")) >= 2:
                        if str(self.out).split("#")[0] == "data":
                            #  "data#" + data_load + "#" + "\r\n";
                            self.data_move = "data#" + str(self.out).split("#")[1]
            ...
            if self.data_sent != "" and self.data_sent != self.data_move and time.time() - self.time_sent > 2:
                print(self.data_sent != "" , self.data_sent, self.data_move , time.time() - self.time_sent > 2)
                ...
                if self.time_sent != "":
                    try:
                        ...
                        print("sent_data----------------", self.data_sent)
                    except:
                        self.connected = False
                        # break
                else:
