...
class process_data_lidar:
    def __init__(self, window_size,window_size_all,scaling_factor,rmse1,rmse2):
        ...
        self.img2 = self.map_all_2d.copy()[int(self.window_size_x_all//2 - self.window_size_y_img2):int(self.window_size_x_all//2 + self.window_size_y_img2),
    ...
    def create_map(self, px, py):
        ...
        px = np.append(px, np.array([int(self.x_goc)]))
        py = np.append(py, np.array([int(self.y_goc)]))
    ...
    def load_map_lan_dau(self):
        ...
        px_0, py_0 = self.transform_agv_points_to_pixel(self.scan_xy, self.rotation, [self.x_goc, self.y_goc])

        mask0 = (px_0 > 0) & (px_0 < self.window_size_x_all) & (py_0 > 0) & (py_0 < self.window_size_y_all)
        px0 = px_0[mask0]
        py0 = py_0[mask0]

        self.arr_goc0 = self.create_map(px0, py0) # đầu vào của icp
        self.update_vi_tri_agv_ban_dau = 0
        return px0, py0
    ...
    def process_data_lidar(self, scan_xy):
        ...
        if self.scan_xy.shape[0] > 1:
            ...
            if self.update_vi_tri_agv_ban_dau == 1 and time.time() - self.time_start > 2 and len(px0) > 60:
                px0, py0 = self.load_map_lan_dau()
            if self.update_vi_tri_agv_ban_dau == 0 and self.arr_goc0.shape[0] > 50:
                ...
                # print("self.vi_tri_agv_new", self.vi_tri_agv_new)
                if webserver.dict_dieu_chinh_vi_tri_agv["setup"] == 0:
                    ...
                    self.rmse = rmse
                    if self.rmse != 0:
                        self.bien_dem = self.bien_dem + 1
                        self.trung_binh = self.trung_binh + rmse
                    ...
                    if rmse != 0:
                        ...
                        self.delta_distan = angle_and_distance.calculate_distance([self.x_goc, self.y_goc],[ new_arr_ok[-1,0], new_arr_ok[-1,1]])
                        self.delta_angle = int(abs(np.arcsin(r[0, 1])) * 180 / np.pi)

                        if rmse < self.rmse1 and self.delta_distan < 40 and self.delta_angle <= 30: # cập nhật vị trí và hướng
                            ...
                            print("------", rmse < self.rmse1, self.delta_distan < 40, self.delta_angle <= 30, rmse, self.delta_distan, self.delta_angle)
                            stop_agv = 1
                        h_img1,w_img1,_ = self.img1.shape
                        # kiểm tra phần trăm trùng khớp
                        so_diem_khop = 0
                        dx = new_arr_ok[:, 0]
                        dy = new_arr_ok[:, 1]
                        for ii in range(0, len(dx)):
                            if (self.map_all[int(dy[ii]), int(dx[ii]), :3].tolist() == [0, 0, 0]):
                                so_diem_khop = so_diem_khop + 1
                        phan_tram = int((so_diem_khop/ len(dx))*100)
                        print("so_diem_khop", so_diem_khop, len(dx), int((so_diem_khop/len(dx))*100))
                        for ii in range(0, len(dx)):
                            # hiển thị kết quả của icp lên ảnh
                            if int(dx[ii]) < w_img1 and int(dy[ii]) < h_img1:
                                cv2.circle(self.img1, (int(dx[ii]), int(dy[ii])), 1, (255, 0, 255), -1)
                            # cập nhật vào map all nếu cần
                            if self.add_all_point == 1:
                                if (rmse <= self.rmse2) or self.add_map == 1:
                                    if int(dy[ii]) < self.window_size_y_all and int(dy[ii]) > 0 and int(dx[ii]) < self.window_size_x_all and int(dx[ii]) > 0:
                                        if (self.map_all[int(dy[ii]), int(dx[ii]), :3].tolist() == [150, 150, 150] or self.add_map == 1):
                                            # --> điểm đang xét là màu xám
                                            self.bresenham_line(self.map_all, 
                                                                int(self.x_goc), int(self.y_goc), 
                                                                int(dx[ii]), int(dy[ii]),  
                                                                step = 30, number_1 = 2, 
                                                                color=self.color, color_check=self.color_check)
                                        elif self.map_all[int(dy[ii]), int(dx[ii]), :3].tolist() == [255, 255, 255]:
                                            # tường dày number
                                            self.bresenham_distan(self.map_all, 
                                                                  int(self.x_goc), int(self.y_goc),
                                                                  int(dx[ii]), int(dy[ii]),
                                                                  number = 6, 
                        ...
                        if rmse <= self.rmse2:
                            ...
                            self.map_all_2d = self.update_occupancy_grid(self.map_all_2d, dx, dy, self.x_goc, self.y_goc)
                        self.add_map = 0
                    else:
                        stop_agv = 1
                ...
                if self.x_goc - self.window_size_x >=0 and self.x_goc + self.window_size_x < self.window_size_x_all and \
                    ...
                    self.img2 = self.map_all_2d.copy()[int(self.y_goc - self.window_size_y_img2):int(self.y_goc + self.window_size_y_img2),
    ...
    def bresenham_line(self, x0, y0, x1, y1):
        """ Trả về danh sách pixel trên đường thẳng bằng thuật toán Bresenham. """
        from skimage.draw import line
        rr, cc = line(y0, x0, y1, x1)
        return cc, rr  # trả về x, y

    def update_occupancy_grid(self, log_odds_map, dx, dy, x_agv, y_agv):
        # Tham số thuật toán OGM
        LOG_ODD_FREE = -1.0
        LOG_ODD_OCCUPIED = 1.5
        LOG_ODD_MIN = -5.0
        LOG_ODD_MAX = 5.0
        OCCUPIED_THRESHOLD = 0.5  # xác suất > 50% thì là tường
        """
        Cập nhật bản đồ log-odds dựa trên dữ liệu quét.
        
        Args:
            log_odds_map (np.ndarray): ma trận log-odds hiện tại.
            dx, dy (list): danh sách tọa độ điểm quét (đã xử lý).
            x_agv, y_agv (int): tọa độ AGV trong ảnh.
        
        Returns:
            np.ndarray: log_odds_map đã cập nhật.
        """
        h, w = log_odds_map.shape
        ...
        for x, y in zip(dx, dy):
            ...
            # Ray từ AGV đến điểm quét
            pxs, pys = self.bresenham_line(x_agv, y_agv, x, y)

            # Trừ điểm cuối ra: free space
            for px, py in zip(pxs[:-1], pys[:-1]):
                if 0 <= px < w and 0 <= py < h:
                    log_odds_map[py, px] += LOG_ODD_FREE
                    log_odds_map[py, px] = np.clip(log_odds_map[py, px], LOG_ODD_MIN, LOG_ODD_MAX)

            # Điểm cuối là occupied
            if 0 <= x < w and 0 <= y < h:
                log_odds_map[y, x] += LOG_ODD_OCCUPIED
                log_odds_map[y, x] = np.clip(log_odds_map[y, x], LOG_ODD_MIN, LOG_ODD_MAX)
